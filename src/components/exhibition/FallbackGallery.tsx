  ...
  return (
    <main id="main-content" className="min-h-screen bg-gallery-paper px-5 pb-10 pt-6 md:px-12">
      <header className="mb-12 flex items-center justify-between text-xs uppercase tracking-[.18em]">
        ...
        <a href={routePath("/guestbook")}>Guestbook</a>
      </header>
      <div className="mx-auto max-w-5xl">
        <p className="mb-4 text-xs text-gallery-muted">Your browser cannot display the 3D halls. Catalogue view is open instead.</p>
        <figure>
        ...
        <div className="mt-8 flex gap-6 text-sm">
          <button onClick={() => go(-1)} type="button">← Previous work</button>
          <button onClick={() => go(1)} type="button">Next work →</button>
        </div>
      </div>
    </main>
  );
